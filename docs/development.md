...
This document contains the technical documentation for the blog repository. The root README is intentionally focused on the public blog, recent articles and navigation.
...
- **Renovate policy** covers npm and GitHub Actions with conservative grouping; auto-merge is explicitly disabled while `main` has no enforced protection/status checks.
...
└── pages/
    ...
    ├── archive.astro
    ├── rss.xml.js
    ├── search.astro           # Pagefind UI + no-JS fallback navigation
    ├── posts/[slug].astro
...
### Previewing drafts and future posts
...
Provider details, environment variables, frontmatter overrides and safety rules are documented in [`publishing.md`](publishing.md).
